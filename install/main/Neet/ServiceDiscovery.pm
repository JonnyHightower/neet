	...
	if ($Protocol eq "tcp"){
		...
		if ($#openPorts >= 0){
			...
			if ($self->haveTCPPort(111) && !defined($RPCInfo)){
				my $err=$MainScan->System("rpcinfo -p $Target > ${outputDir}/rpcinfo.txt 2>/dev/null");
			...
			for my $line (@openPorts){
				...
				if (! -f "${outputDir}/banners/${port}.txt"){
					...
					if (($port == 139) || ($port == 445)){
						$MainScan->System("smbclient -L //$Target/ -U \"\" -N -p $port 2> ${outputDir}/banners/${port}.txt 1>&2");
